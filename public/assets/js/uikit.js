...
(function() {
    ...
    function dattacodbnd(source, blacklist) {
        ...
        if (blacklist) {
            source = source.replace(/className="([^"]+)"/g, function(m, clsStr) {
    ...
    function Dattaopnmdl(src, formattedSrc) {
        document.querySelector('.datta-example-modal-content').innerHTML = '<pre><code className="hljs html xml">' + formattedSrc + '</code></pre>';
